...
# ... (VARIABLES EXPORTADAS Y ESTADOS OMITIDOS POR BREVEDAD - SON IGUALES) ...
...
func _ready():
	randomize()
	# ... (READY OMITIDO POR BREVEDAD - SON IGUALES) ...
...
# ... (PROCESS Y PHYSICS_PROCESS OMITIDOS POR BREVEDAD - SON IGUALES) ...
		...
		State.MOVING_TO_TREE:
			if is_instance_valid(nav_agent):
				...
				if not nav_agent.is_target_reachable():
					push_warning("[Lenador] Objetivo inalcanzable, volviendo a IDLE.")
					_on_tree_depleted() 
					return
...
# ... (NAVEGACIÓN OMITIDA POR BREVEDAD - SON IGUALES) ...

func _on_velocity_computed(safe_velocity: Vector2):
	velocity = safe_velocity
	move_and_slide()

## 🧭 Lógica de Búsqueda Corregida (Con Debug)
func _find_nearest_tree():
	...
	shape_query.shape.radius = search_radius
	shape_query.collide_with_areas = true
	shape_query.collide_with_bodies = true
	
	var results = space_state.intersect_shape(shape_query)
	
	# 💡 DEPURACIÓN: Comprobar si la consulta física devuelve algo
	if debug and results.is_empty():
		print("[Lenador DEBUG] Query ejecutada, 0 nodos detectados en radio %d." % search_radius)
	...
	for result in results:
		...
		if is_instance_valid(node) and node.is_in_group("arbol") and node.has_method("gather_resource"):
			# Comprobación de estado del árbol.
			if node.is_dead: 
				continue 
...
func _start_navigation():
	if is_instance_valid(target_tree) and is_instance_valid(nav_agent):
		nav_agent.set_target_position(target_tree.global_position)
...
func _gather(delta: float):
	...
	if gather_timer >= gather_rate:
		...
		if is_instance_valid(target_tree) and target_tree.is_in_group("arbol"):
			...
			if debug:
				print("Recolectando %d de madera." % wood_gained)
...
func _on_tree_depleted():
	if is_instance_valid(target_tree):
		if target_tree.is_connected("depleted", _on_tree_depleted):
...
# Función auxiliar para la animación y flip del sprite
