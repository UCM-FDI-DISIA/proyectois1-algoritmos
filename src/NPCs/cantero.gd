...
var target_mine: MinaPiedraAnimado = null
var is_ready = false
var debug := true
...
# ============================================================
# 🌑 Encontrar mina más cercana
# ============================================================
func _find_nearest_mine():
	var mines = get_tree().get_nodes_in_group("mina")
	var nearest_mine: MinaPiedraAnimado = null
	var min_distance = INF
	var map_rid = nav_agent.get_navigation_map()

	for mine in mines:
		var mine_obj := mine as MinaPiedraAnimado
		if not mine_obj: continue
		if mine_obj.is_depleted: continue

		var raw = mine_obj.global_position
...
func _is_target_in_gather_range() -> bool:
	if not is_instance_valid(target_mine): return false
...
func _gather(delta: float):
	...
	# Sumar piedra al ResourceManager
	if is_instance_valid(resource_manager):
		resource_manager.add_resource("stone", target_mine.PIEDRA_POR_GOLPE)

	# Si completó los golpes, "talamos" la mina
	if chop_count >= chop_needed:
		if debug: print("[Cantero] Mina agotada (3 golpes).")
