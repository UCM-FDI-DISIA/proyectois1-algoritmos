...
En la carpeta actual se pueden ver distintas subcarpetas para descargar un ejecutable de Feudalia en cada Plataforma. Esta opción no depende del entorno de desarrollo (Godot engine 4.5.).

1. Descargue la versión correspondiente a su dispositivo. En esa carpeta encontrará: dos ejecutables (el juego y la consola) y un archivo .pck.
	- Un archivo ejecutable con el juego (Windows: .exe, Linux: .x86_64).
	- Un archivo .pvk.
	- Un archivo ejecutable adicional de consola/depuración (Windows: .console.exe, Linux: .sh)
...
* Acceso permitido a conexiones WebSocket (ws) hacia el servidor de GDSync. Si su red (firewall, proxy, red corporativa, etc.) bloquea WebSockets (como por jemplo, la red de la UCM) o el acceso al servidor, el modo PVP puede no funcionar.
...
En el repositorio de GitHub, se encuentra el código del proyecto completo, ejecutable en Godot 4.5.
...
2. Descargar el repositorio completo en una carpeta local (por ejemplo X/).
3. Abrir Godot 4.5.
4. Importar el proyecto seleccionando la carpeta X/ (la que contiene un .godot).
